...
// Booking thread
void* processBooking(void* arg) {
    BookingArgs* ba = (BookingArgs*)arg;
...
// Handler: SEATMAP|train|date|from|to|class
...
int main() {
    ...
    while (1) {
        ...
        pthread_t tid;
        if (pthread_create(&tid, NULL, clientHandler, (void*)(intptr_t)client_sock) != 0) {
